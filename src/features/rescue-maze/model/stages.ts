...
export const RESCUE_MAZE_STAGES: readonly StageDefinition[] = [
  ...
  makeStage({
    ...
    rows: ['P.KD..', '.###S.', 'A...#.', '###.H#', 'T....E'],
    entities: (rows) => [
      { kind: 'key', id: 'stage-11-star-key', keyId: 'star', symbol: '★', position: positionOf(rows, 'K') },
      { kind: 'door', id: 'stage-11-star-door', keyId: 'star', symbol: '★', position: positionOf(rows, 'D') },
      {
        kind: 'switch',
        id: 'stage-11-sun-switch',
        bridgeId: 'stage-11-sun-bridge',
        symbol: '☀',
        activation: 'player',
        position: positionOf(rows, 'S'),
      },
      {
        kind: 'bridge',
        id: 'stage-11-sun-bridge',
        symbol: '☀',
        position: positionOf(rows, 'H'),
      },
      animal(rows, 'A', 'stage-11-bird', 'とり', 'bird'),
      treasure(rows, 'stage-11-ruby'),
    ],
    parMoves: 32,
    bonusGoals: ['under-par', 'all-treasures'],
  }),
  makeStage({
    id: 'rescue-12',
    name: 'レスキュー チャンピオン',
    rows: ['P.KD..', '.###.C', 'A.XS#.', '##.H..', 'T.B#.E'],
    entities: (rows) => [
      { kind: 'key', id: 'stage-12-star-key', keyId: 'star', symbol: '★', position: positionOf(rows, 'K') },
      { kind: 'door', id: 'stage-12-star-door', keyId: 'star', symbol: '★', position: positionOf(rows, 'D') },
      { kind: 'box', id: 'stage-12-box', position: positionOf(rows, 'X') },
      {
        kind: 'switch',
        id: 'stage-12-moon-switch',
        bridgeId: 'stage-12-moon-bridge',
        symbol: '☾',
        activation: 'box',
        position: positionOf(rows, 'S'),
      },
      {
        kind: 'bridge',
        id: 'stage-12-moon-bridge',
        symbol: '☾',
        position: positionOf(rows, 'H'),
      },
      animal(rows, 'A', 'stage-12-dog', 'いぬ', 'dog'),
      animal(rows, 'B', 'stage-12-panda', 'ぱんだ', 'panda'),
      animal(rows, 'C', 'stage-12-bird', 'とり', 'bird'),
      treasure(rows, 'stage-12-ruby'),
    ],
    parMoves: 40,
    bonusGoals: ['under-par', 'all-treasures'],
  }),
